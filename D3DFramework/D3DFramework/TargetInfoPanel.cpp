...
void TargetInfoPanel::Render()
{
    ...
    RenderManager::DrawUI(TextureKey::UI_HP_BAR_01, barPos, 0);
    barPos.x += 80;
    barPos.y += 8;
    ...
    if (generation == 1)
    {
        RenderManager::DrawUI(TextureKey::UI_FACE_POKEMON_1ST, barPos, pokeNumber - 1);
    }
    else if (generation == 2)
    {
        pokeNumber -= (int)Pokemon::Mew;
        pokeNumber += 27;//¾È³ó °¹¼ö
        RenderManager::DrawUI(TextureKey::UI_FACE_POKEMON_2ND, barPos, pokeNumber - 1);
    }
    else
    {
        pokeNumber -= (int)Pokemon::Celebi;
        RenderManager::DrawUI(TextureKey::UI_FACE_POKEMON_3RD, barPos, pokeNumber - 1);
    }

    // ·¹º§ Ç¥½Ã
    WCHAR wstr3[8] = {};
    wsprintf(wstr3, L"Lv.%d", (int)target->stat.level);
    RenderManager::DrawFont(wstr3, barPos.x + 150, barPos.y + 90, D3DCOLOR_ARGB(255, 255, 255, 255));
}
